...
def calc_heating_coil(Qhsf, Qhsf_0, Ta_sup_hs, Ta_re_hs, Ths_sup_0, Ths_re_0, ma_sup_hs, ma_sup_0, Ta_sup_0, Ta_re_0):
    ...
    if Q > 0 and ma_sup_hs > 0:
        ...
        #tho, ch = np.vectorize(substation.calc_HEX_heating)(Q, UA_heating, thi, tco, tci, cc) ORIGINAL
        tho, ch = substation.calc_HEX_heating(Q, UA_heating, thi, tco, tci, cc)
    else:
        thi = np.nan
        tho = np.nan
        ch = 0.0
...
def calc_cooling_coil(Qcsf, Qcsf_0, Ta_sup_cs, Ta_re_cs, Tcs_sup_0, Tcs_re_0, ma_sup_cs, ma_sup_0, Ta_sup_0, Ta_re_0):
    ...
    if Q > 0 and ma_sup_cs > 0:
        ...
        tco, cc = substation.calc_HEX_cooling(Q, UA_cooling, thi, tho, tci, ch)
    else:
        tco = np.nan
        tci = np.nan
        cc = 0.0
