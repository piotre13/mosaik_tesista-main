...
import warnings
import numpy as np
import buildings.demand.airconditioning_model as airconditioning_model
...
import buildings.demand.space_emission_systems as space_emission_systems
...
#the only part I'm intrested in*******************************************************************
            ...
            #print(bpr.name, 'temperature int: ', rc_model_temperatures['T_int'])
...
def calc_heat_loads_radiator(bpr, t, tsd):
   ...
   #print(bpr.name,' Qhs_sen_sys: ', tsd['Qhs_sen_sys'][t])
    # write temperatures to rc-model
    #print(bpr.name,'>>>',tsd['T_int'][t],'prima di rc_temperatures_to_tsd')
    rc_temperatures_to_tsd(rc_model_temperatures, tsd, t)
    #print(bpr.name,'>>>',tsd['T_int'][t],'dopo di rc_temperatures_to_tsd >> should be: ',rc_model_temperatures['T_int'])
...
def calc_cool_loads_mini_split_ac(bpr, t, tsd):
    ...
    # (0) Extract values from tsd
    # get values from tsd
    if t == 0: # questo non dovrebbe servire perchè partiamo sempre dal 1 gennaio heating season
        t_int_prev = 20.
        x_int_prev = 0.6
    else:
        t_int_prev = tsd['T_int'][t - 1]
...
def calc_rc_no_loads(bpr, tsd, t):
    ...
    tsd['theta_o'][t] = rc_model_temperatures['theta_o']
    #TODO
    update_tsd_no_cooling(tsd, t)#SET VALUES AT 0 CAUSE NO LOADS
    update_tsd_no_heating(tsd, t)#SET VALUES AT 0 CAUSE NO LOADS
...
def calc_rc_heating_demand(bpr, tsd, t):
   ...
   #print(t_int_0)
    ...
    if not rc_model_SIA.has_sensible_heating_demand(t_int_0, tsd, t):
        ...
        t_int_set = float(tsd['ta_hs_set'][t])
        #print(t_int_set,' >> T_set')
        ...
        raise Exception("Unexpected status in 'calc_rc_heating_demand'")
    #print(bpr.name,' >> phiACT',phi_h_act)
...
def calc_rc_cooling_demand(bpr, tsd, t):
    ...
    if not rc_model_SIA.has_sensible_cooling_demand(t_int_0, tsd, t):
        ...
        t_int_set = float(tsd['ta_cs_set'][t])
