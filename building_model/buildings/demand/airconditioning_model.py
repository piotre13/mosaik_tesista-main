...
from buildings.demand.latent_loads import convert_rh_to_moisture_content, total_moisture_in_zone
...
def electric_humidification_unit(g_hu, m_ve_mech):
    ...
    if g_hu > 0.0:
        ...
        e_hs_lat_aux = 15.0 * m_ve_mech * float(SECONDS_PER_HOUR)  # assuming a performance of 15 W por Kg/h of humidified air source: bertagnolo 2012
...
def local_air_recirculation_unit_cooling(qc_sen_demand_aru, g_dhu_demand_aru, t_int_prev, x_int_prev, bpr, t_control,
    ...
    g_dhu_aru_max = np.min(
        [(total_moisture_in_zone(bpr, x_sup_c_aru_max) - total_moisture_in_zone(bpr, x_int_prev)) / float(SECONDS_PER_HOUR), 0.0])  #
