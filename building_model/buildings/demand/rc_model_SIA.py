...
from cea.constants import SECONDS_PER_HOUR
...
#DONE>> should be checked the /3.6 that's hourly referred, how to change with a constants???
def calc_h_ea(m_ve_mech, m_ve_window, m_ve_inf_simple):
    cp = 1.005 / 3.6  # (Wh/kg/K)
    # TODO: check units of air flow

    # get values
    m_v_sys = m_ve_mech * SECONDS_PER_HOUR  # mass flow rate mechanical ventilation
    m_v_w = m_ve_window * SECONDS_PER_HOUR  # mass flow rate window ventilation
    m_v_inf = m_ve_inf_simple * SECONDS_PER_HOUR  # mass flow rate infiltration
...
def calc_T_int(phi_a, theta_ea, theta_c, h_ac, h_ea):
    ...
    #print('(',h_ac,' * ' , theta_c,' + ', h_ea,' * ', theta_ea,' + ', phi_a,') / (', h_ac,' + ',h_ea,')')
...
def calc_rc_model_temperatures(phi_hc_cv, phi_hc_r, bpr, tsd, t):
    ...
    #print('theta_m t1 at t: ',t,' : ',theta_m_t_1)
    ...
    c_m = bpr.rc_model['Cm'] / SECONDS_PER_HOUR  # (Wh/K) SIA 2044 unit is Wh/K, ISO unit is J/K

    #print(Ea, El, Epro, Htr_op, Htr_w, I_sol, Qs, T_ext, a_m, a_t, a_w, c_m, m_ve_inf,
...
#DONE here the cal function are used also the one for Tint
def _calc_rc_model_temperatures(Eaf, Elf, Epro, Htr_op, Htr_w, I_sol, Qs, T_ext, a_m, a_t, a_w, c_m,
    ...
    phi_i_a = calc_phi_i_a(Eaf=Eaf, Epro=Epro) # include processes #DOESN'T SEEM
    ...
    phi_m_tot = calc_phi_m_tot(phi_m, phi_a, phi_c, theta_ea, theta_em, theta_ec, h_1, h_2, h_3, h_ec, h_ea, h_em) #PHI TOT MAYBE THIS EQ IS USEFUL
...
def has_heating_demand(bpr, tsd, t):
    ...
    #return True #just changed alway a demand meand always make calculations...
...
def has_sensible_cooling_demand(t_int_0, tsd, t):
    ...
    # True, if temperature w/o conditioning is higher than cooling set point temperature, else False
    return t_int_0 > float(ta_cs_set) + temp_tolerance
...
def has_sensible_heating_demand(t_int_0, tsd, t):
    ...
    # True, if T_int < ta_hs_set, False, if T_int >= ta_hs_set
    return t_int_0 < float(ta_hs_set) - temp_tolerance
    #return True
